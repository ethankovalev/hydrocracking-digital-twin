...
- **Separation sweep**: the reference was rerun at 8 A, 6 A, and 1 A. All
...
- **Molecule-in-a-box check**: a direct isolated-molecule calculation
  (ethane in an empty ~15x15x15 A cell) gave the same E_ads and C-C length
...
correction on the uncorrected 4.10 A geometry captures only ~40% of the
well depth compared to letting the geometry relax under the added forces.

| | uncorrected | with D3 |
|---|---|---|
| E_ads [eV] | +0.0032 | -0.308 |
| closest contact [A] | 4.10 | 2.78 |
| C-C bond [A] | 1.53 | 1.53 |

Adding D3 pulled the molecule in by ~1.3 A and produced a binding energy
...
- **Coverage not converged.** The 3x3 unit cell places ethane ~3.5 A from
